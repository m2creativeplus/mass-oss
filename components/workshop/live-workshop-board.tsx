...
export function LiveWorkshopBoard({ orgId = "mass-hargeisa" }: { orgId?: string }) {
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case "urgent": return <Badge className="bg-red-600 text-[10px] px-1.5">URGENT</Badge>
      case "high": return <Badge className="bg-orange-500 text-[10px] px-1.5">HIGH</Badge>
      case "normal": return <Badge className="bg-blue-500 text-[10px] px-1.5">NORMAL</Badge>
      default: return <Badge variant="outline" className="text-[10px] px-1.5">LOW</Badge>
    }
  }
    ...
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
          ...
          <div className={`text-2xl font-bold ${(kpis?.avgPartsDelayMinutes ?? 0) > 60 ? 'text-rose-500' : 'text-slate-900 dark:text-white'}`}>
            ...
            return (
              <div key={col.key} className="w-64 flex-shrink-0 flex flex-col">
                ...
                <div className="bg-slate-100 dark:bg-slate-900/50 flex-1 rounded-b-lg border border-t-0 border-slate-200 dark:border-slate-800 p-2 space-y-2 overflow-y-auto max-h-[500px]">
                  {colJobs.length === 0 && (
                    <div className="text-center text-xs text-slate-400 py-8">No jobs</div>
                  )}
                  {colJobs.map(job => (
                    <div
                      key={job._id}
                    ...
                    >
                      ...
                      <div className="text-[11px] text-slate-500 space-y-1">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="truncate">{job.services?.[0] || "Service"}</span>
                        </div>
                        <div className="flex items-center gap-1">
                      ...
                      <div className="flex gap-1 mt-2 pt-2 border-t border-slate-100 dark:border-slate-700">
                        {col.key === "check-in" && (
                          <Button
                            ...
                            onClick={() => autoAssign({ workOrderId: job._id as any, orgId })}
                          >
                            <Zap className="h-3 w-3 mr-1" /> Auto-Assign
                          </Button>
                        )}
                        {(col.key === "in-progress" || col.key === "diagnosis") && (
                          <Button
                            size="sm"
                            className="h-6 text-[10px] bg-rose-500 hover:bg-rose-600 text-white flex-1"
                            onClick={() => markPartsReq({ workOrderId: job._id as any })}
                          >
                            <Package className="h-3 w-3 mr-1" /> Need Parts
                          </Button>
                        )}
                        {col.key === "waiting-parts" && (
                          <Button
                            ...
                            onClick={() => markPartsRcv({ workOrderId: job._id as any })}
                          >
                            <CheckCircle2 className="h-3 w-3 mr-1" /> Parts Arrived
                          </Button>
                        )}
                      </div>
                    </div>
      ...
      <div className="mt-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-3">
        ...
        <div className="flex gap-3 overflow-x-auto pb-1">
          {technicians.map(tech => {
            ...
            return (
              <div key={tech._id} className="flex-shrink-0 w-32 bg-slate-50 dark:bg-slate-800 rounded-lg p-2 border border-slate-200 dark:border-slate-700">
                ...
                <div className="mt-1 w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${
                      pct >= 100 ? 'bg-red-500' : pct >= 66 ? 'bg-orange-400' : 'bg-emerald-500'
                    }`}
                    style={{ width: `${Math.min(pct, 100)}%` }}
                  />
                </div>
                <div className="text-[10px] text-slate-400 mt-0.5">{load}/{max} jobs</div>
              </div>
            )
          })}
        </div>
