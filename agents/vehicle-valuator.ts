...
async function valuateBatch(vehicles: typeof VEHICLES) {
  ...
  const CHUNK_SIZE = 10;
  const allValuations: any[] = [];

  for (let i = 0; i < vehicles.length; i += CHUNK_SIZE) {
    const chunk = vehicles.slice(i, i + CHUNK_SIZE);
    console.log(`  🤖 Gemini valuating batch ${i / CHUNK_SIZE + 1} (${chunk.length} vehicles)...`);
...
${chunk.map((v, idx) => `${idx + 1}. ${v.make} ${v.model} (${v.yearRange})`).join("\n")}
    ...
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`,
        { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { temperature: 0.15, maxOutputTokens: 8192 } }) })
      ...
      if (match) {
        try { allValuations.push(...JSON.parse(match[0])) } catch { }
      }
    } catch (e) {
      console.error("Batch failure:", e)
    }

    await new Promise(r => setTimeout(r, 3000)); // Rate limit prevention
  }
